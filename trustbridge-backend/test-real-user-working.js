...
class RealUserWorkingSimulator {
  constructor() {
    this.results = {
      successful: [],
      failed: [],
  ...
  async simulateAssetOwnerJourney() {
    ...
    try {
      // Step 1: Create Asset (Skip registration for now)
      await this.log('Asset Owner', '1.1', 'Creating coffee farm asset...');
      const assetData = {
      ...
      if (assetResponse.status === 200 || assetResponse.status === 201) {
        ...
        await this.log('Asset Owner', '1.1', `Asset created successfully (ID: ${this.assetId})`);
      }

      // Step 2: Submit for Verification
      await this.log('Asset Owner', '1.2', 'Submitting asset for verification...');
      ...
      if (verificationResponse.status === 200 || verificationResponse.status === 201) {
        ...
        await this.log('Asset Owner', '1.2', `Verification submitted (ID: ${this.verificationId})`);
      }

      // Step 3: Check Verification Status
      await this.log('Asset Owner', '1.3', 'Checking verification status...');
      const statusResponse = await axios.get(`${BASE_URL}/verification/status/${this.assetId}`);
      if (statusResponse.status === 200) {
        const status = statusResponse.data.data;
        await this.log('Asset Owner', '1.3', `Verification status: ${status.status} (Score: ${status.score}%)`);
      }
  ...
  async simulateInvestorJourney() {
    ...
    try {
      // Step 1: Browse Available Assets
      await this.log('Investor', '3.1', 'Browsing available investment opportunities...');
      ...
      if (assetsResponse.status === 200) {
        ...
        await this.log('Investor', '3.1', `Found ${assets.length} total assets, ${verifiedAssets.length} verified`);
      }

      // Step 2: Create Investment
      await this.log('Investor', '3.2', 'Creating investment in coffee plantation...');
      const investmentData = {
        userId: "0x4567890123456789012345678901234567890123",
        assetId: this.assetId,
        amount: 50000,
        tokens: 100
      };
      ...
      if (investmentResponse.status === 200 || investmentResponse.status === 201) {
        ...
        await this.log('Investor', '3.2', `Investment created (ID: ${this.investmentId}) - $50,000 for 100 tokens`);
      }

      // Step 3: Check Portfolio
      await this.log('Investor', '3.3', 'Checking investment portfolio...');
      const portfolioResponse = await axios.get(`${BASE_URL}/portfolio?userId=0x4567890123456789012345678901234567890123`);
      if (portfolioResponse.status === 200) {
        const portfolio = portfolioResponse.data.data || portfolioResponse.data;
        await this.log('Investor', '3.3', `Portfolio value: $${portfolio.totalValue || 0}`);
      }
  ...
  async simulateBuyerJourney() {
    ...
    try {
      // Step 1: Browse Assets for Purchase
      await this.log('Buyer', '4.1', 'Browsing assets available for purchase...');
      const assetsResponse = await axios.get(`${BASE_URL}/assets`);
      if (assetsResponse.status === 200) {
        const assets = assetsResponse.data.data || assetsResponse.data;
        const verifiedAssets = assets.filter(asset => asset.status === 'VERIFIED');
        await this.log('Buyer', '4.1', `Found ${verifiedAssets.length} verified assets available for purchase`);
      }

      // Step 2: Check Real-time Market Data
      await this.log('Buyer', '4.2', 'Checking real-time market data...');
      ...
      if (btcResponse.status === 200 && ethResponse.status === 200) {
        ...
        await this.log('Buyer', '4.2', `Market data - BTC: $${btcPrice}, ETH: $${ethPrice}`);
      }

      // Step 3: Check Analytics
      await this.log('Buyer', '4.3', 'Reviewing market analytics...');
      ...
      if (analyticsResponse.status === 200) {
        ...
        await this.log('Buyer', '4.3', `Market TVL: $${analytics.totalValueLocked || 0}`);
      }
  ...
  async simulatePlatformFeatures() {
    ...
    try {
      ...
      const feeds = ['BTC_USD', 'ETH_USD', 'LINK_USD', 'HBAR_USD'];
      for (const feed of feeds) {
        try {
          ...
          if (response.status === 200) {
            ...
            const source = response.data.data?.source || response.data.source || 'Chainlink';
            await this.log('Platform', 'F.1', `${feed}: $${price} (${source})`);
          }
        } catch (error) {
  ...
  async runCompleteSimulation() {
    console.log('🚀 TRUSTBRIDGE REAL USER SIMULATION (WORKING FEATURES)');
    console.log('=' .repeat(60));
...
async function main() {
  const simulator = new RealUserWorkingSimulator();
