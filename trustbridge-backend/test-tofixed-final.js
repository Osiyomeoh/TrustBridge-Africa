...
/**
 * Test script to verify toFixed(8) approach (final)
 */

console.log('🧪 Testing toFixed(8) Approach (Final)\n');
...
testPrices.forEach(price => {
  const platformFee = Number.parseFloat(((price * platformFeePercent) / 100).toFixed(8));
  const royaltyAmount = Number.parseFloat(((price * royaltyPercentage) / 100).toFixed(8));
  const sellerAmount = Number.parseFloat((price - platformFee - royaltyAmount).toFixed(8));
